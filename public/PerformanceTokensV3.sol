...
contract PerformanceTokensV3 is ERC20, ReentrancyGuard {
    ...
    constructor(address _token, string memory name_, string memory symbol_, address _owner, address _revMgr) ERC20(name_, symbol_) {
        token = _token;
        owner=_owner;
        _name=name_;
        _symbol=symbol_;
    ...
    function setRevenueManager(address _revMgr) public onlyOwner() {
        revMgr = _revMgr;
    }
    ...
    function withdrawFee() public {
        ...
        ERC20(token).transfer(revMgr, what);
        perfEarned=0;
        totalEarned+=what;
    }
